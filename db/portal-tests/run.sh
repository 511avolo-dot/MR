...
if command -v node >/dev/null 2>&1; then
  echo "▶ تأكيدات حارس الملفات المرفوعة (JS)"
  node "$HERE/file-guard.test.mjs" | tail -1
fi
...
echo ""
echo "✅ كل اختبارات البوابة نجحت (113 تأكيداً: 8 صادر + 7 أمان + 5 ميزانية + 5 آيبان + 7 مطابقة + 4 مرتجعات + 5 عملات + 5 عقود + 3 ترسية مجزّأة + 3 تكامل مرتجع/فاتورة + 23 سيناريو دورة حياة + 12 سيناريو متقدّم + 5 تغطية أدوار + 7 إعادة/تعديل + 5 إعادة فتح تعميد + 9 إلزام مستند العرض) + 18 تأكيد حارس ملفات."
