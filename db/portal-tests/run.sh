...
echo "▶ أقلّ امتياز على portal_users: RLS بدور authenticated + دليل آمن + منع التصعيد (P0-1)"
"${PSQL[@]}" -d "$DB" -f "$HERE/38_portal_users_least_privilege.sql"

echo ""
echo "✅ كل اختبارات البوابة نجحت (202 تأكيداً: 8 صادر + 8 أمان + 5 ميزانية + 5 آيبان + 7 مطابقة + 4 مرتجعات + 5 عملات + 5 عقود + 3 ترسية مجزّأة + 3 تكامل مرتجع/فاتورة + 23 سيناريو دورة حياة + 12 سيناريو متقدّم + 5 تغطية أدوار + 7 إعادة/تعديل + 5 إعادة فتح تعميد + 9 إلزام مستند العرض + 10 محرّك الصرف + 8 حوكمة متقدّمة + 4 ميزانية الصرف + 7 سجلّ مستفيدين + 4 اعتماد جماعي + 7 صرف متكرّر + 6 اعتماد بريد الصرف + 5 تدقيق مقاوم للعبث + 4 إشعارات معامَلاتية + 3 تصليب anon + 6 إصلاحات Codex [AUTHZ-01/GOV-01/قسم نشط] + 19 مستندات الصرف المباشر [062+round3/4] + 5 أقلّ امتياز portal_users [P0-1]) + 18 تأكيد حارس ملفات + 7 تأكيدات نقطة رفع وثائق التسجيل."
