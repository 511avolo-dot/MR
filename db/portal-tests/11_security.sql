...
BEGIN
  ...
  IF v_bad <> 'portal_audit_write, portal_award_total, portal_budget_committed, portal_contract_consumed, portal_create_token, portal_invoiced_total, portal_outbox_claim, portal_outbox_mark, portal_outbox_purge, portal_pr_transition_email, portal_returns_total, portal_run_sla, portal_supplier_token_request'
    THEN RAISE EXCEPTION 'S7 fail: المجموعة الخادمية غير متوقّعة (%): %', v_cnt, v_bad; END IF;
  RAISE NOTICE 'PASS S7 المجموعة الخادمية = 13 دالة مقصودة، الباقي للمستخدم';
