...
BEGIN
  ...
  -- (p0_2c) مدير قسم الطلب مباشرةً، أو مدير قطاعه (مدير أيّ قسم في نفس القطاع).
  ...
  -- من يُلغي: الأدمن/المشتريات (أي وقت) — المُقدّم قبل بدء التعميد — أو مدير القسم/القطاع
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 043 (تعديل الطلب المُعاد للتصحيح) — مطابقة لـ db/portal-migrations/043
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ⚠️ لازم: Supabase تمنح anon صلاحية صريحة على كل دالة جديدة (pg_default_acl)،
--    و REVOKE ... FROM public لا يزيل منحاً صريحاً لدور. فيجب السحب من anon أيضاً.
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 044 (إعادة فتح التعميد من طور الصرف) — مطابقة لـ db/portal-migrations/044
-- ═══════════════════════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION portal_payment_transition(p_payment_id bigint, p_action text,
    p_comment text DEFAULT NULL, p_return_to text DEFAULT NULL, p_details jsonb DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 049 (إلزام مستند العرض + رفع المورّد لعرضه) — مطابقة لـ db/portal-migrations/049
-- ═══════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 050 (محرّك الصرف الموحّد الواعي بالدورة) — مطابقة لـ db/portal-migrations/050
-- ═══════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 051 (idempotency + إبطال saga) — مطابقة لـ db/portal-migrations/051
-- ═══════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 052 (ضبط ميزانية الصرف المستقلّ) — مطابقة لـ db/portal-migrations/052
-- ═══════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 053 (سجلّ المستفيدين + ضبط تغيير آيبان المستفيد) — مطابقة لـ db/portal-migrations/053
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 054 (الاعتماد الجماعي) — مطابقة لـ db/portal-migrations/054
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 055 (الصرف المتكرّر/المجدوَل) — مطابقة لـ db/portal-migrations/055
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 056 (الاعتماد بالبريد لدورة الصرف) — مطابقة لـ db/portal-migrations/056
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 057 (تدقيق مقاوم للعبث — Hash-Chained WORM) — مطابقة لـ db/portal-migrations/057
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 058 (إشعارات معامَلاتية للاعتماد) — مطابقة لـ db/portal-migrations/058
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 060 (AUTHZ-01 ربط قسم الصرف + GOV-01 ميزانية الصرف المتكرّر) — مطابقة لـ db/portal-migrations/060
-- ═══════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
--  دمج الهجرة 061 (تصليب جولة Codex الثانية: قسم نشط + قفل ميزانية + دقّة + تحديث مستفيد + تدقيق تخطٍّ)
-- ═══════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════════════
...
REVOKE ALL ON FUNCTION portal_reqdoc_guard() FROM anon, PUBLIC, authenticated;
-- P0-parity — فهرس تغطية مفتاح أجنبي على portal_request_documents(supersedes_id) (جدول 062).
...
-- The legacy RPC signature is retained for client compatibility; p_amount is no
-- longer trusted, and p_supplier_name is only a selector for an approved split award.
...
BEGIN
  ...
  -- Row lock serializes concurrent invoices for the same award so the remaining
  -- authoritative amount cannot be consumed twice.
  SELECT * INTO v_award
