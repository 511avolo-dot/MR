-- ════════════════════════════════════════════════════════════════════
--  إعادة ضبط وحدات سير العمل لتطابق كود التطبيق بدقّة
--  آمن: الجداول الأربع جديدة وفارغة. شغّله في Supabase SQL Editor.
-- ════════════════════════════════════════════════════════════════════

-- حذف نسخة Claude Code المختلفة (دوال + جداول)
DROP FUNCTION IF EXISTS submit_supplier_rfq_quote CASCADE;
DROP FUNCTION IF EXISTS generate_request_no CASCADE;
DROP FUNCTION IF EXISTS generate_rfq_no CASCADE;
DROP FUNCTION IF EXISTS get_rfq_for_supplier(text,text) CASCADE;
DROP FUNCTION IF EXISTS submit_supplier_quote(text,text,jsonb,jsonb,jsonb,text,boolean) CASCADE;
DROP FUNCTION IF EXISTS submit_supplier_quote(text,text,jsonb,jsonb,jsonb,text) CASCADE;
DROP TABLE IF EXISTS proc_rfq_quotes CASCADE;
DROP TABLE IF EXISTS proc_rfqs CASCADE;
DROP TABLE IF EXISTS proc_purchase_requests CASCADE;
DROP TABLE IF EXISTS proc_notifications CASCADE;

-- إعادة الإنشاء بالمخطط القانوني (المطابق للتطبيق):
-- ════════════════════════════════════════════════════════════════════
