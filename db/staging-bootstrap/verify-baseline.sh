...
#  P0-1b يُطبّق قبل اختبار 38 فقط لإثبات إغلاق تجاوز session_user بدون كسر بقية الحزمة.
# ════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
for f in $(ls "$ROOT"/db/portal-tests/[0-9]*.sql | sort); do
  ...
  if [[ "$b" == "38_portal_users_least_privilege.sql" && "$a_p0b" = "0" ]]; then
    "${PSQL[@]}" -d "$DBA" -f "$P0B" >/tmp/p0b_a.log 2>&1 || { echo "❌ فشل تطبيق P0-1b في المرحلة أ"; cat /tmp/p0b_a.log; exit 1; }
    a_p0b=1
  fi
...
for f in $(ls "$ROOT"/db/portal-tests/[0-9]*.sql | sort); do
  ...
  if [[ "$b" == "38_portal_users_least_privilege.sql" && "$b_p0b" = "0" ]]; then
    "${PSQL[@]}" -d "$DBB" -f "$P0B" >/tmp/p0b_b.log 2>&1 || { echo "❌ فشل تطبيق P0-1b في المرحلة ب"; cat /tmp/p0b_b.log; exit 1; }
    b_p0b=1
  fi
  "${PSQL[@]}" -d "$DBB" -f "$f" >/tmp/phaseB.log 2>&1 || { echo "❌ المرحلة ب فشل $b:"; grep -iE "ERROR|EXCEPTION" /tmp/phaseB.log | head -5; exit 1; }
  b_pass=$((b_pass+1))
done
echo "   ✓ المرحلة ب: $b_pass ملفّ نجح على (أساس+062+P0-1b)"

echo "▶ [7] ✅ F1 proof PASS — Phase A(061)=$a_pass نجح/$a_skip مؤجَّل · 062 ✓ · P0-1b ✓ · Phase B=$b_pass نجح — خروج 0"
