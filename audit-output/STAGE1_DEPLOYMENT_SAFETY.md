...
| F1 · baseline determinism (drift guard) | `node scripts/deploy/build-baseline.mjs --check` | **LOCAL PASS (2026-08-09; exact branch content, CI pending)** — sha256 `38d54fddac6b18ff8396ee8d8f23661fe1366ab807d2fc20069950c6fd8b6a5f` reproducible from `portal-standalone.sql` |
| F1 · launcher three-mode + mode-confusion + checksum drift | `node scripts/stage1-tests.mjs` | **PASS — 61 assertions** (bootstrap/apply-062/apply-remediations dry-run, no-`--mode` fails closed, prod-ref rejected, pinned baseline+062+12-file P0 chain sha == files) |
...
- **Live `supabase db push`** — `--mode bootstrap` (apply baseline), then `--mode apply-062` (assert exactly 062 pending), then `--mode apply-remediations` (assert exactly the ordered P0-1b…P0-1n chain) against isolated staging. **NOT RUN as a clean rebuild.**
...
| ID | Finding | Fix + proof |
|---|---|---|
| **G1-R3-01** (P0) | `supabase db push --project-ref` is not a valid CLI contract (that flag is on `supabase link`); adapter was string-tested only, and inheriting an existing link reintroduces target ambiguity | Fixed launcher `scripts/deploy/supabase-push.mjs`: **isolated workdir (no inherited repo link)** → a SHA-pinned, versioned baseline+062+P0 remediation payload → `supabase link --project-ref <validated ref>` (password via `SUPABASE_DB_PASSWORD` env) → **verify the linked ref == validated ref** → assert the exact pending set for the selected phase → `supabase db push --linked`. The default named adapter constructs `--mode apply-remediations`; bootstrap and 062 remain explicit prerequisite phases. Real CLI-contract check parses actual `--help`. Negatives: production ref, missing mode/password, hash drift, missing/extra migration, and linked-ref mismatch all fail closed. |
...
- **launchers + net-allow:** `supabase-push` dry-run builds the SHA-verified baseline, 062, and ordered 12-file P0 remediation payload in three explicit phases + isolated `link→verify→push --linked` plan (never prod) · rejects `GUARDED_REF=prod` · every pinned SHA equals its source file · live path rejects missing password and fails closed without the CLI · `e2e-run` and `net-allow` retain the production/other-ref network denial controls.
