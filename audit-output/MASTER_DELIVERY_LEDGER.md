...
| S1-GUARD-COUPLE | P1 | deploy | command-coupled environment guard — validated target must be the target the command uses | Stage 1 | ⚠️ **PARTIAL (Stage 1, repo-only) — design + local logic tested; real exec gated (G1-R6).** `supabase-db-push` builds the **full migration history** from `manifest.json` (62 files, per-file SHA-256; 062=`20260730120000` after live 061; G1-R6-01/02) → `init`→`link`→verify→`db push --dry-run --linked`→**assert exactly `{062}` pending** (G1-R5-02)→`db push --linked` (**live NOT run** — staging bootstrapped from manifest, owner-gated). CI `supabase-contract` installs **pinned `2.110.0`** + enforces `--help` (G1-R5-01). Raw psql adapter **removed** (G1-R4-02). `browser-e2e`→**`browser-run.mjs`** (real Playwright context.route + smoke journey; `run.mjs` delegates; G1-R6-03/04) — **NOT run** (no package/staging). Pending Gate 1 |
