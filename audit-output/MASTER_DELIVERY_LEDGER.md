...
| S1-GUARD-COUPLE | P1 | deploy | command-coupled environment guard — validated target must be the target the command uses | Stage 1 | **implemented + G1-R2-01-hardened (Stage 1, repo-only) — `env-guard.mjs` uses named adapters that build the target internally (no arbitrary passthrough); `migrate`/`e2e` require `--command`; rejects `--exec`/`sh -c`/`--project-ref <prod>`/`--db-url db.<prod>`; `--dry-run` proof; pending Gate 1** |
