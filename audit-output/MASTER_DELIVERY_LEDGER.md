...
| S1-GUARD-COUPLE | P1 | deploy | command-coupled environment guard — validated target must be the target the command uses | Stage 1 | **implemented + G1-01-hardened (Stage 1, repo-only) — `env-guard.mjs --exec` substitutes `{GUARDED_REF}` AND rejects any explicit mismatched target before spawning (marker-proof negative test); pending Gate 1** |
...
| PAGES-DEPLOY | P1 | deploy | GitHub Pages published Function-dependent pages (404 on `/api/*`) | Stage 1 | **implemented + G1-03/G1-04-hardened (Stage 1, repo-only) — per-page `needs_functions` manifest + set-equality `--check` (catches omitted/stale pages) + query+hash-preserving redirect stub; `invite.html`/`register-portal.html` added; tested; pending Gate 1** |
| CFG-ENV-G1-02 | P1 | deploy | `PORTAL_ENV` could override branch (preview→prod bypass); key not bound to project | Stage 1 | **implemented (Stage 1, repo-only) — branch authoritative + `PORTAL_ENV` conflict ⇒ 409 + key project-ref binding + branch-absent identity requirement; 7 negative tests; pending Gate 1** |
