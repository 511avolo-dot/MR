...
| S1-GUARD-COUPLE | P1 | deploy | command-coupled environment guard — validated target must be the target the command uses | Stage 1 | **implemented (Stage 1, repo-only) — `env-guard.mjs --exec` injects `GUARDED_REF`; tested; pending Gate 1** |
...
| PAGES-DEPLOY | P1 | deploy | GitHub Pages published Function-dependent System-3 pages (404 on `/api/*`) | Stage 1 | **implemented (Stage 1, repo-only) — `deploy/system3-manifest.json` + `scripts/pages-exclude.mjs` in `pages.yml` (redirect stub); tested; pending Gate 1** |
