...
| S1-GUARD-COUPLE | P1 | deploy | command-coupled environment guard — validated target must be the target the command uses | Stage 1 | ⚠️ **PARTIAL (Stage 1, repo-only) — design + local logic tested; real exec gated (G1-R4).** `supabase-db-push` launcher builds a **SHA-verified 062 payload** in an isolated workdir → `init`→`link --project-ref`→verify linked==ref→`db push --dry-run --linked`→`db push --linked` (**live apply NOT run** — needs pinned CLI + staging; CI job `supabase-contract` installs the CLI + enforces the `--help` contract). **Raw psql adapter REMOVED (G1-R4-02).** `browser-e2e`: Node-fetch guard is harness-only; **real browser boundary `browser-run.mjs` (Playwright context.route) NOT run** (no package/staging) — result open. Pending Gate 1 |
