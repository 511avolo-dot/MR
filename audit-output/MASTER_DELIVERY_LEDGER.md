...
| S1-GUARD-COUPLE | P1 | deploy | command-coupled environment guard — validated target must be the target the command uses | Stage 1 | **implemented + G1-R2-01/G1-R3-01/02/03-hardened (Stage 1, repo-only) — adapters delegate to fixed launchers: `supabase-push` (isolated workdir → `link --project-ref` → verify linked==ref → `db push --linked`, correct CLI contract, contract-verify script) · `psql-migration` (PGPASSWORD env, host from ref, TLS) · `browser-e2e` (fixed launcher + network allowlist blocking non-target/prod Supabase). No arbitrary passthrough/`--spec`; `--dry-run` + malicious-scenario negatives; pending Gate 1** |
