...
12. **Owner-directed remediation + re-test (2026-07-28, rev 2).** Owner chose: fix the code defects now; keep admin as
    ...
    assertions** (178 SQL + 18 file-guard + 7 endpoint — the 6th endpoint case pins the closed doc-type allowlist).
...
## 2026-07-29 — G0-01 CLOSED (live-verified)
- Supabase MCP re-authorized; ran read-only `list_migrations` on `mwbjoysuybgbrvfrprex`. Live tail: 057,058,**059,060,061**; **062 absent**. → 059/060/061 applied live (VERIFIED); 062 NOT applied (VERIFIED absent). Corrected 061 from "presumed not applied" to APPLIED. Ledger/inventory/PR body/reconciliation updated. **No production change (read-only query).** Remaining Gate-0 items: G0-05 owner DoA threshold confirmation + owner independent recheck.
