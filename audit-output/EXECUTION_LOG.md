...
12. **Owner-directed remediation + re-test (2026-07-28, rev 2).** Owner chose: fix the code defects now; keep admin as
    ...
    relied on a cross-department write, now forbidden — rewritten to an OPS user). Re-ran suite → **EXIT 0, 200
    assertions** (177 SQL + 18 file-guard + 5 endpoint); `node --check reg-doc.js` OK. Rewrote the audit docs to verdict
    **READY WITH CONDITIONS** (0 open HIGH). Migration `060` is in the repo, **pending live apply** (Supabase MCP
