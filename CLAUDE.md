...
`005-resubmit.sql` → `006-guard-alignment.sql` → `007-doa-po-chain.sql` → `008-flexible-return.sql`
(007 يستبدل بذور DoA داخلياً بأمان: يفكّ ارتباط التعميدات ثم يعيد الزرع — لا تحذف شيئاً يدوياً.)

### 🔜 المتبقّي (نفس الصرامة — الخطة في المهام 16–18):
- **C (H1)** فرض RLS للرؤية (own/sector/all) + **إبقاء رؤية كل معتمِد للطلبات المعنيّة به** (requisition/award/po).
