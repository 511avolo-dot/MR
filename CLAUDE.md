...
`005-resubmit.sql` → `006-guard-alignment.sql` → `007-doa-po-chain.sql`
(007 يستبدل بذور DoA داخلياً بأمان: يفكّ ارتباط التعميدات ثم يعيد الزرع — لا تحذف شيئاً يدوياً.)

### 🔜 المتبقّي (نفس الصرامة — الخطة في المهام 15–18):
- **B** إعادة توجيه مرنة عند الرفض/الإرجاع (لأي مشارك سابق؛ من عاد إليه يعيده لمن سبقه) — الدورة الأولى + المالية.
