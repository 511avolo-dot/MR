...
// ── المطبوعات تلتزم معجم `.print-*` (القاعدة 3) ──
// بلاغ المالك 2026-09-15: «محضر طلب الشراء سيء جدا جدا من ناحية عرضه». الجذر
// أنّ `prPrint` كانت تبني جداول `border="1"` بأنماط سطريّة مخترَعة بدل المعجم
// الموحّد، فتخرج بحدود المتصفّح الافتراضية بلا ترويسة ولا إجماليات ولا تواقيع.
{
  const body = CODE.slice(CODE.indexOf('async function prPrint('),
    CODE.indexOf('\n}', CODE.indexOf('async function prPrint(')) + 2);
  T('محضر الطلب لا يبني جداول border="1" خام', !/border="1"/.test(body), 'عاد لجدول خام');
  T('محضر الطلب يستعمل معجم print-* الموحّد',
    /print-table/.test(body) && /print-parties/.test(body) && /print-signatures/.test(body),
    'ينقصه معجم المطبوعات');
  T('محضر الطلب يمرّ بـprintDocOpen لا window.open',
    /printDocOpen\(/.test(body) && !/window\.open\(/.test(body), 'لا يمرّ بخطّ الطباعة الموحّد');
  // التواقيع من المعتمِدين الفعليّين: محضرٌ يزعم خانة توقيع لدور لم يقرّر شيئاً وثيقةٌ مضلِّلة.
  T('تواقيع المحضر مشتقّة من سلسلة الاعتماد لا مسمّيات ثابتة',
    /prSignatories\(pr\)/.test(body), 'تواقيع ثابتة');
  T('المحضر يعرض سلسلة القرارات', /سلسلة القرارات المسجّلة/.test(body), 'بلا سلسلة قرارات');
}

// أوامر الشراء المرتبطة تُقرأ من مصدر واحد يسقط إلى `po_number` للصفوف السابقة
// لجدول الروابط — وإلّا عُرِض طلبٌ مرتبط فعلاً بأنّه «لم يُربط أمر شراء بعد».
T('لا قراءة مباشرة لـpo_links في عرض الارتباطات',
  /function prWorkspaceLinksHTML\(pr\)\{\s*const links=prPoLinksOf\(pr\);/.test(CODE),
  'يقرأ pr.po_links مباشرةً');
   ...
   **الحقيقيّتين** — صندوق القسم 2 يكعّبهما فلا يصلح لاختبار البوّابة نفسها. */
{
  ...
  /* القرار التشغيلي المحدّث: اعتماد الحاجة ثم إذن بدء التسعير، داخل سجل الطلب نفسه. */
  T('سلسلة الموافقات الإلكترونية ظاهرة داخل مساحة متابعة الطلب',
    /سلسلة الموافقات/.test(CODE)
    && /اعتماد الاحتياج/.test(CODE)
    && /السماح بالتسعير/.test(CODE));
...
/* ── ٣٢) اعتماد إلكتروني + مرفقات داعمة + ربط أمر الشراء ─────────
   النموذج التشغيلي الحالي يجمع بوابتي القرار مع متابعة التسعير والتنفيذ
   في سجل واحد، ويُبقي المرفقات داعمة للقرار المسجّل إلكترونياً. */
{
   ...
   الملكية وهي حراسة **نطاق** لا **صلاحية**. */
{
  ...
  T('وبوّابة الزرّ = بوّابة الفعل: تبويبا الرفع يختفيان بلا can_create_pr',
    ...
    && /canCreatePR\?`<button[^`]+prGoView\('create'\)/.test(CODE)
    && /hasPermission\('can_create_pr'\)\?`<button[^`]+prGoView\('templates'\)/.test(CODE));
  T('والوجهة نفسها محروسة فلا تُفتَح بحالة قديمة',
    /if\(!canCreatePR && \(view==='create' \|\| view==='templates'\)\) view\s*=\s*'list'/.test(CODE)
    && /function prGoView[\s\S]{0,240}requirePermission\('can_create_pr'/.test(CODE));

  /* 🐛 بلاغ المالك 2026-09-16: «رفع النموذج والقراءة الآلية المفروض يتاح للجميع
     — بعض الموظفين عند الرفع يأتيهم تنبيه عدم وجود صلاحية».
     الجذر: `prParseForm` كان محروساً بـ`can_use_ai` (بوّابة **تحليلات** تمسح
     الكتالوج والموردين)، وهي غائبة عن مفاتيح الميدان الستّة في `staff-invite.js`
     و`hasPermission` صارمة للمُنطَّق ⇒ كل موظّف ميدانيّ مدعوّ يُمنَع **داخل شاشة
     فتحها بحقّ**. الزرّ لا يقرأ إلا نموذج صاحب الطلب نفسه، فبوّابته = بوّابة
     الشاشة. ⚠️ ولا يجوز أن تعود `can_use_ai`: المفتاح لا يصل متصفّح المُنطَّق
     أصلاً (قائمة مفاتيح `proc_settings` البيضاء تحجب `ai_config`)، والوسيط
     الخادميّ `/api/ai` هو ما يخدمه — فالحارس الصحيح هو صلاحية الفعل لا الأداة. */
  T('وقراءة النموذج آلياً ببوّابة الشاشة (can_create_pr) لا ببوّابة التحليلات',
    /async function prParseForm\(\)\{[\s\S]{0,160}requirePermission\('can_create_pr'/.test(CODE)
    && !/async function prParseForm\(\)\{[\s\S]{0,160}requirePermission\('can_use_ai'/.test(CODE));
