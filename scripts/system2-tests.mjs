...
G('٢٩) حملة التسجيل + إكمال بطاقة المورد');
{
  ...
  T('الرابط في سطر مستقلّ (وإلّا لم يصر قابلاً للنقر)',
    msg.split('\n').some(l => l.trim() === URL_));
  T('الرسالة تُطمئن أن الملف الحالي يُحدَّث لا يُستبدَل', /ملف جديد/.test(msg) && /ملفكم الحالي/.test(msg));
  ...
  T('اسم فارغ لا يُنتج رسالة مبتورة', regCampMessage({}, URL_).includes('شركاءنا الكرام'));
  ...
  T('البطاقة تُنبّه أن الملف الحالي يُحدَّث لا يُستبدَل', /فلا ملف جديد/.test(CARD));
