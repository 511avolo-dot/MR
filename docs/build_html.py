...
AXES = [
    ...
    ("سابعاً: التكامل مع الإدارة المالية", [
        ("بطء التحويل للموردين وتعليق المعاملات", "مدة ملزمة (٣–٥ أيام)، وتصعيد للإدارة العليا عند التجاوز"),
...
CONTROLS = [
    ("المشتريات الطارئة", "بإذن مدير العمليات، واستكمال المنافسة لاحقاً"),
    ("وحيد المصدر", "بنموذج تبرير فني/تجاري معتمد"),
    ("المشتريات المتكررة", "عقود إطارية سنوية مع موردين معتمدين"),
    ("الفصل بين الواجبات", "فصل الطلب والتقييم والاعتماد والدفع"),
    ("حفظ السجلات", "لا تقل عن خمس سنوات لكل الملفات"),
    ("التدقيق الدوري", "نصف سنوي على المنافسة والموافقات والأرشفة"),
]

PROC = [
    ("١) طلب الشراء", "تحديد الحاجة والمواصفات + تحقق مالي مسبق", "الجهة الطالبة", "يومان"),
    ("٢) طلب العروض", "دعوة ثلاثة موردين مؤهلين على الأقل", "المشتريات", "٣–٥ أيام"),
    ("٣) التحليل والمفاضلة", "مطابقة فنية وجدول مقارنة", "المشتريات", "يوم"),
    ("٤) تعميد المورد", "اعتماد الأفضل قيمة بعد ثلاثة عروض", "مدير العمليات", "يوم"),
    ("٥) أمر الشراء", "إصدار الأمر وإرساله للمورد", "المشتريات", "يوم"),
    ("٦) الصرف/التحويل", "الدفع بعد أمر الشراء (تحويل/نقاط بيع)", "المالية", "—"),
    ("٧) التوريد والاستلام", "فحص الكمية والجودة ومحضر استلام", "المستلم/المستودعات", "يوم"),
    ("٨) المطابقة والإغلاق", "مطابقة المستندات وأرشفة الملف", "المالية والمشتريات", "يوم"),
]

FLOW = ["طلب الشراء", "طلب ثلاثة عروض", "التحليل والمفاضلة", "تعميد المورد",
        "أمر الشراء", "الصرف/التحويل", "التوريد والاستلام", "المطابقة والإغلاق"]
...
ROADMAP = [
    ...
    ("حتى ٩٠ يوماً", "a", ["أتمتة سجل الأسعار", "العقود الإطارية", "عهدة وسائق للمشتريات", "معالجة عهدة قصر الياسمين"]),
    ("المدى البعيد", "i", ["نظام مشتريات موحّد", "لوحات متابعة", "تقييم الموردين", "استراتيجية التوريد"]),
]

KPIS = [
    ("التسليم في الموعد", "26.2٪", "90٪ فأكثر"),
    ("الأوامر المتأخرة", "15.4٪", "أقل من 5٪"),
...
PR_TILES = [
    ("585", "صنف مسجّل", "579 مُسعّراً · 6 بلا سعر", "i"),
    ("864", "سجل سعري", "تاريخ أسعار مرجعي للمناقصات", "gold"),
    ("25", "فئة", "تغطّي مواد المشاريع", "g"),
    ("53", "مورد معتمد", "شركاء نشطون بسجلّاتهم", "i"),
]
...
TILES = [
    ("57", "أمر شراء", "52 نشطاً · 5 ملغاة", "i"),
    ("351", "ألف ريال", "قيمة المشتريات الفعلية", "gold"),
    ("8", "أوامر متأخرة", "15.4٪ من النشطة", "r"),
    ("20.4", "يوم متوسط التأخير", "أقصى 59 يوماً", "r"),
    ("10", "عالقة لدى المالية", "بانتظار التحويل", "a"),
    ("26.2٪", "سُلّمت في موعدها", "11 من 42 مُسلَّماً", "r"),
]
STATUS = [("مُسلَّم كامل", 42, "g"), ("لدى المالية", 10, "a"), ("ملغى", 5, "r")]
AGING = [("0 – 7 أيام", 2, "g"), ("8 – 14 يوم", 1, "a"),
         ("15 – 30 يوم", 2, "a"), ("أكثر من 30 يوم", 3, "r")]
SECTORS = [("الصيانة والتشغيل", 28, "i"), ("الإنشاءات", 11, "a"),
           ("الإدارة العامة", 10, "g"), ("النقليات", 3, "gold")]
BIG3 = [("10", "أوامر عالقة لدى المالية", "بانتظار التحويل", "r"),
        ("20.4", "يوم متوسط التأخير", "أقصى 59 يوماً", "r"),
        ("26.2٪", "سُلّمت في موعدها", "11 من 42 مُسلَّماً", "r")]
...
def build():
    pr_tiles = "".join(
        f"<div class='tile {c}'><div class='big'>{e(b)}</div><div class='tl'>{e(l)}</div><div class='ts'>{e(s)}</div></div>"
        for b, l, s, c in PR_TILES)
    pr_section = f"""<section class="slide"><div class="part">إنجاز المشتريات · الوضع الراهن</div>
<h2 class="t">إنجاز سجل الأسعار</h2>
<div class="tiles" style="grid-template-columns:repeat(4,1fr)">{pr_tiles}</div>
<div class="urg" style="background:#f6f1e6;border-color:var(--gold)">
<b style="color:var(--gold)">من الصفر إلى منظومة مؤتمتة:</b> عند استلام الإدارة لم يكن هناك سجل أسعار، وكان عدد الموردين المستلَمين ٤ فقط؛ واليوم: نظام مشتريات فعّال ومؤتمت، و٥٣ مورداً معتمداً في ٢٥ فئة، و٥٨٥ صنفاً و٨٦٤ سجلاً سعرياً يُحدَّث باستمرار.</div>
<div class="src">المصدر: نظام المشتريات — سجل الموردين وسجل الأسعار</div></section>"""
    ax_list = [axis_html(t, r) for t, r in AXES]
    ax_list.insert(3, pr_section)   # بعد محور تسعير المناقصات
    axes = "".join(ax_list)
    pols = "".join(f"<div class='pol'><h4>{e(t)}</h4><p>{e(d)}</p></div>" for t, d in POLICIES)
    proc = "".join(f"<tr><td class='st'>{e(a)}</td><td>{e(b)}</td><td class='ow'>{e(c)}</td><td class='du'>{e(d)}</td></tr>" for a, b, c, d in PROC)
    controls = "".join(f"<div class='pol'><h4>{e(t)}</h4><p>{e(d)}</p></div>" for t, d in CONTROLS)
    flow = "".join(f"<div class='step{' key' if i in (1,2,3) else ''}'>{e(x)}</div>"
    ...
    kpis = "".join(
        ...
        f"<div class='ct'><span class='cur'><small>الحالي</small>{e(cur)}</span>"
        f"<span class='ar2'>←</span><span class='tgt'><small>الهدف</small>{e(g)}</span></div></div>"
        for t, cur, g in KPIS)
...
table.pr tr:nth-child(even) td{{background:#f1ece2}} .st{{color:var(--ink);font-weight:700}} .ow{{text-align:center;color:var(--ink)}} .du{{text-align:center;color:var(--green);font-weight:700;white-space:nowrap}}
...
.kpi .ct{{display:flex;align-items:center;justify-content:center;gap:8px;padding:14px 6px}}
.kpi .cur,.kpi .tgt{{display:flex;flex-direction:column;font-weight:700;font-size:18px}}
.kpi .cur{{color:var(--red)}} .kpi .tgt{{color:var(--green)}}
.kpi .cur small,.kpi .tgt small{{color:var(--mute);font-weight:400;font-size:11px}}
.kpi .ar2{{color:var(--gold);font-weight:700;font-size:20px}}
...
<div class="c amber"><h3>الأثر</h3>تأخّر تسليم ٧٤٪ من الأوامر، و١٠ عالقة لدى المالية، وتآكل ثقة الموردين.</div>
...
<div class="src">المصدر: نظام المشتريات — سجل أوامر الشراء (٥٧ أمراً)</div></section>

<section class="slide"><div class="part">الجزء الأول · الوضع الراهن</div><h2 class="t">حالة الأوامر والأولوية</h2>
<div class="two">
<div class="chartbox"><h3>حسب الحالة</h3>{bars(STATUS,42)}</div>
<div class="chartbox"><h3>تقادم الأوامر المتأخرة (8)</h3>{bars(AGING,3)}
<div class="urg"><b>تأخير متفاقم:</b> من ٨ أوامر متأخرة، ٥ تجاوزت ١٥ يوماً و٣ منها أكثر من ٣٠ يوماً.</div></div>
</div>
<div class="src">المصدر: نظام المشتريات — سجل أوامر الشراء (٥٧ أمراً)</div></section>
...
<div class="chartbox"><h3>أوامر الشراء حسب القطاع</h3>{bars(SECTORS,28)}</div>
<div class="src">١٠ أوامر عالقة لدى المالية، وثلاثة أرباع المُسلَّم تأخّر — المصدر: نظام المشتريات (٥٧ أمراً)</div></section>

{axes}

<section class="slide"><div class="part">مقترح دليل السياسات والإجراءات — الإصدار 1.1</div><h2 class="t">سياسات المشتريات الحاكمة</h2>
...
<section class="slide"><div class="part">مقترح دليل السياسات والإجراءات — الإصدار 1.1</div><h2 class="t">إجراءات الدورة ومددها</h2>
<table class="pr"><thead><tr><th>المرحلة</th><th>الإجراء</th><th>المسؤول</th><th>المدة</th></tr></thead><tbody>{proc}</tbody></table></section>
...
<div class="sub2">الاستثناءات (باعتماد مدير المشتريات): النثريات — المتعاقد عليه إطارياً — الطوارئ المعتمدة مسبقاً.</div></section>

<section class="slide"><div class="part">مقترح دليل السياسات والإجراءات — الإصدار 1.1</div><h2 class="t">المسارات الخاصة والرقابة</h2>
<div class="pols">{controls}</div></section>
