...
def page(s, idx=None):
    pass  # ترقيم الصفحات يُضاف في تمريرة نهائية بعد اكتمال الشرائح
...
def source_note(s, text='المصدر: نظام المشتريات — سجل أوامر الشراء (57 أمراً)'):
...
cards = [
    ...
    ('الأثر', 'تأخّر تسليم 74٪ من الأوامر، و10 عالقة لدى المالية، وتآكل ثقة الموردين.', AMBER),
...
tiles = [
    ('57', 'أمر شراء', INK, '52 نشطاً · 5 ملغاة'),
    ('351', 'ألف ريال', GOLD, 'قيمة المشتريات الفعلية'),
    ('8', 'أوامر متأخرة', RED, '15.4٪ من النشطة'),
    ('20.4', 'يوم متوسط التأخير', RED, 'أقصى 59 يوماً'),
    ('10', 'عالقة لدى المالية', AMBER, 'بانتظار التحويل'),
    ('26.2٪', 'سُلّمت في موعدها', RED, '11 من 42 مُسلَّماً'),
]
cw = (SW - Inches(1.2)) / 3
...
# لوحة 2) حالة الأوامر وتقادم التأخير
s = new(); brandbar(s, 'لوحة حالة الأوامر والتأخير', 'توزيع أوامر الشراء')
...
hbars(s, RX, Inches(1.95), RW, [
    ('مُسلَّم كامل', 42, GREEN), ('لدى المالية', 10, AMBER), ('ملغى', 5, RED),
], maxv=42, lblw=Inches(1.9))
txt(s, LX, Inches(1.35), LW, Inches(0.4), 'تقادم الأوامر المتأخرة (8)', size=16,
    color=GOLD, bold=True, align=PP_ALIGN.RIGHT)
hbars(s, LX, Inches(1.95), LW, [
    ('0 – 7 أيام', 2, GREEN), ('8 – 14 يوم', 1, AMBER),
    ('15 – 30 يوم', 2, AMBER), ('أكثر من 30 يوم', 3, RED),
], maxv=3, lblw=Inches(2.1))
rect(s, LX, Inches(4.35), LW, Inches(1.0), RGBColor(0xF7, 0xE9, 0xE7), line=RED, lw=1.5)
txt(s, LX + Inches(0.2), Inches(4.46), LW - Inches(0.4), Inches(0.8),
    [{'t': 'تأخير متفاقم', 'size': 15, 'color': RED, 'bold': True, 'space': 4},
     {'t': 'من 8 أوامر متأخرة: 5 تجاوزت 15 يوماً، و3 منها أكثر من 30 يوماً.', 'size': 14, 'color': INK}],
    align=PP_ALIGN.RIGHT)
decision(s, 'لم يُسلَّم في موعده سوى 26.2٪ من الأوامر، و10 أوامر عالقة لدى المالية — خلل في زمن الدورة.')
source_note(s)
page(s)

# لوحة 3) أين يضيع الوقت
s = new(); brandbar(s, 'أين يضيع الوقت؟ تشخيص التأخير', 'مكامن التأخير الأبرز')
big = [
    ('10', 'أوامر عالقة لدى المالية', 'بانتظار التحويل', RED),
    ('20.4', 'يوم متوسط التأخير', 'أقصى 59 يوماً', RED),
    ('26.2٪', 'سُلّمت في موعدها', '11 من 42 مُسلَّماً', RED),
]
cw = (SW - Inches(1.2)) / 3
...
txt(s, SW - Inches(6.6), Inches(3.75), Inches(6.3), Inches(0.4),
    'أوامر الشراء حسب القطاع (عدداً)', size=16, color=GOLD, bold=True, align=PP_ALIGN.RIGHT)
hbars(s, Inches(0.5), Inches(4.25), Inches(12.3), [
    ('الصيانة والتشغيل', 28, INK), ('الإنشاءات', 11, AMBER),
    ('الإدارة العامة', 10, GREEN), ('النقليات', 3, GOLD),
], maxv=28, lblw=Inches(3.0), bar_h=Inches(0.34), gap=Inches(0.16))
decision(s, 'تتركّز الأوامر في الصيانة والتشغيل (28 من 52)، و10 عالقة لدى المالية — أولوية: إلزام المالية بمدة التحويل.')
source_note(s)
page(s)
...
TOTAL = len(slides)
# ترقيم الصفحات: تمريرة نهائية بالعدد الصحيح (عدا الغلاف والخاتمة)
