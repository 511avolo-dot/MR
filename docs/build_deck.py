...
# أرقام سجل الأسعار (من النظام الحيّ — تُعبّأ القيمتان أدناه عند توفّرهما)
PR_ITEMS = '—'    # عدد البنود المسجّلة في سجل الأسعار
PR_PRICES = '—'   # عدد الأسعار المرجعية المسجّلة
...
axes = [
    ...
    ('سابعاً: التكامل مع الإدارة المالية', [
        ('بطء التحويل للموردين وتعليق المعاملات', 'مدة ملزمة (3–5 أيام)، ودفعات أسبوعية، وتصعيد للإدارة العليا عند التجاوز'),
...
def price_register_slide():
    s = new(); brandbar(s, 'إنجاز سجل الأسعار', 'ما بنته المشتريات للوصول إليه')
    tiles = [
        (PR_ITEMS, 'بند مسجّل', INK, 'في سجل الأسعار'),
        (PR_PRICES, 'سعر مرجعي', GOLD, 'للرجوع إليها في المناقصات'),
        ('26', 'فئة', GREEN, 'تغطّي مواد المشاريع'),
        ('42', 'مورد مسجّل', INK, 'بسجلّات وبيانات تواصل'),
    ]
    cw = (SW - Inches(1.2)) / 4
    for i, (big, lbl, co, sub) in enumerate(tiles):
        x = SW - Inches(0.5) - (i + 1) * cw - i * Inches(0.07) + Inches(0.07)
        kpi_tile(s, x, Inches(1.55), cw, Inches(1.95), big, lbl, co, sub)
    rect(s, Inches(0.5), Inches(3.85), SW - Inches(1.0), Inches(1.5), CARD, line=LINE)
    rect(s, SW - Inches(0.5) - Pt(8), Inches(3.85), Pt(8), Inches(1.5), GOLD)
    txt(s, Inches(0.7), Inches(4.0), SW - Inches(1.4), Inches(1.25),
        [{'t': 'من الصفر إلى منظومة', 'size': 16, 'color': GOLD, 'bold': True, 'space': 5},
         {'t': 'عند استلام الإدارة لم يكن هناك سجل أسعار، وكان عدد الموردين المستلَمين 4 فقط؛', 'size': 15, 'color': INK, 'bold': True, 'space': 3},
         {'t': 'واليوم أصبح لدينا 42 مورداً في 26 فئة وسجل أسعار مرجعي يُبنى ويُحدَّث باستمرار.', 'size': 15, 'color': SLATE}],
        align=PP_ALIGN.RIGHT)
    source_note(s, 'المصدر: نظام المشتريات — سجل الموردين وسجل الأسعار')
    page(s)
...
steps = ['نشوء الاحتياج', 'طلب رسمي\nبالمواصفات', 'فحص المستودع', 'طلب ثلاثة\nعروض',
         'المقارنة', 'الاعتماد حسب\nالصلاحية', 'أمر الشراء', 'الصرف/التحويل',
         'الاستلام\nوالفحص', 'الإغلاق']
...
right = [
    ...
    ('9) الإشعار والتعاقد', 'إشعار المورد بالشروط', 'المشتريات'),
    ('10) الصرف/التحويل', 'تحويل المستحق (غالباً قبل التوريد)', 'المالية'),
    ('11) التوريد', 'متابعة التسليم', 'المشتريات'),
    ('12) الاستلام والفحص', 'فحص المطابقة للمواصفات', 'المستودعات'),
    ('13) المطابقة', 'مطابقة المستندات', 'المشتريات'),
